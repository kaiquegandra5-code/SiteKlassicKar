      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-5 font-display text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-white">
            Precisa de uma <span className="text-lime-400">lavagem pontual</span>?
          </h2>
            ...
            return (
              <article
                ...
                className={`rounded-2xl border border-white/10 bg-white/[0.02] p-6 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.04] ${
                  s.highlight ? 'ring-1 ring-lime-500/30 lg:scale-[1.02]' : ''
                }`}
              >
                <div className="relative flex items-center justify-between">
                  <span
                    className={`grid h-12 w-12 place-items-center rounded-xl bg-white/5 border border-white/10 transition-all ${
                      s.highlight ? 'text-lime-400' : 'text-zinc-300'
                    }`}
                  >
                    <Icon className="h-6 w-6" />
                  </span>
                  <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider ${s.highlight ? 'bg-lime-500/10 text-lime-400 border border-lime-500/20' : 'bg-white/5 text-zinc-400 border border-white/10'}`}>
