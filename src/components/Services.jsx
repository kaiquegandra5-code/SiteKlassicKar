      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-5 font-display text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-white">
            Lave mais, <span className="text-lime-400">pague menos</span> e mantenha seu carro sempre impecável
          </h2>
            ...
            return (
              <article
                ...
                className={`rounded-2xl border border-white/10 bg-white/[0.02] flex flex-col p-6 transition-all duration-300 hover:border-white/20 hover:bg-white/[0.04] relative ${
                  isHighlight ? 'ring-1 ring-lime-500/30 lg:scale-[1.02]' : ''
                }`}
              >
                {/* Header: ícone + badge */}
                <div className="relative flex items-start justify-between gap-3">
                  <span className={`grid h-12 w-12 place-items-center rounded-xl bg-white/5 border border-white/10 transition-all ${isHighlight ? 'text-lime-400' : 'text-zinc-300'}`}>
                    <Icon className="h-6 w-6" />
                  </span>
                  <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider ${isHighlight ? 'bg-lime-500/10 text-lime-400 border border-lime-500/20' : 'bg-white/5 text-zinc-400 border border-white/10'}`}>
                ...
                <ul className="mt-4 space-y-1.5 flex-1">
                  {s.features.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-xs text-zinc-300">
                      <Check className="h-3.5 w-3.5 text-lime-400 flex-shrink-0 mt-0.5" />
                      <span>{f}</span>
                    </li>
                  ))}
                </ul>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {s.sizes.map((size, idx) => (
                      ...
                      >
                        ...
                        <div className={`font-display text-sm sm:text-base font-bold mt-0.5 ${idx === 1 ? 'text-lime-400' : 'text-lime-300'}`}>
                          {size.price}
                        </div>
                      </div>
                    ))}
